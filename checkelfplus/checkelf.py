...
def check_intersections(result_dependencies, package_deps, package_name, systembase, systemdevel):
    ...
    result_section = list(set([re.sub(CMPFILTR, r"\1", x) for x in result_deps]) - set([re.sub(CMPFILTR, r"\1", x) for x in package_deps]))
    ...
    cmp_func = lambda x, y: len(re.sub(CMPFILTR, r"\1", x)) - len(re.sub(CMPFILTR, r"\1", y))
    result_lists = itertools.izip_longest(sorted(sorted(list(set(package_deps))), cmp=cmp_func),
                                          sorted(sorted(result_deps), cmp=cmp_func),
                                          sorted(sorted(result_section), cmp=cmp_func),
                                          fillvalue="")
    return result_lists
...
def output_result(package_name, package_dir):
    ...
    for package_file in package_files:
        ...
        if re.search("LSB\s+shared object", package_file_info):
            package_elf_files.append(os.path.realpath(package_file))
        elif re.search("LSB\s+executable", package_file_info):
